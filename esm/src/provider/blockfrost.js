...
export class Blockfrost {
    constructor(url, projectId, userAgent) {
        ...
        this.defaultHeaders = {
            ...
            lucid,
            "User-Agent": userAgent || "lucid-sdk",
        };
    }
    ...
    async getUtxos(addressOrCredential) {
        ...
        let page = 1;
        while (true) {
            const pageResult = await fetch(`${this.url}/addresses/${queryPredicate}/utxos?page=${page}`, { headers: this.defaultHeaders }).then((res) => res.json());
            if (pageResult.error) {
    ...
    async getUtxosWithUnit(addressOrCredential, unit) {
        ...
        let page = 1;
        while (true) {
            const pageResult = await fetch(`${this.url}/addresses/${queryPredicate}/utxos/${unit}?page=${page}`, { headers: this.defaultHeaders }).then((res) => res.json());
            if (pageResult.error) {
                if (pageResult.status_code === 404) {
                    return [];
                }
    ...
    async getUtxoByUnit(unit) {
        const addresses = await fetch(`${this.url}/assets/${unit}/addresses?count=2`, { headers: this.defaultHeaders }).then((res) => res.json());
    ...
    async getUtxosByOutRef(outRefs) {
        ...
        const utxos = await Promise.all(queryHashes.map(async (txHash) => {
            const result = await fetch(`${this.url}/txs/${txHash}/utxos`, { headers: this.defaultHeaders }).then((res) => res.json());
    ...
    async getDelegation(rewardAddress) {
        const result = await fetch(`${this.url}/accounts/${rewardAddress}`, { headers: this.defaultHeaders }).then((res) => res.json());
        if (!result || result.error) {
    ...
    awaitTx(txHash, checkInterval = 3000, timeout = 60000) {
        ...
        return (await Promise.all(result.map(async (r) => ({
            ...
            scriptRef: r.reference_script_hash
                ? (await (async () => {
                    ...
                    const { cbor: script } = await fetch(`${this.url}/scripts/${r.reference_script_hash}/cbor`, { headers: this.defaultHeaders }).then((res) => res.json());
                    return {
